...
def load_recovery():
    try:
        df = pd.read_csv(APP_DIR / "data_recovery.csv")
        # Ensure required columns exist
        if 'branch' in df.columns and 'customer' in df.columns and 'decline_amount' in df.columns:
            return df
        return pd.DataFrame()
...
def show_recovery():
    ...
    has_equip = 'equipment' in f.columns
    has_acct = 'acct' in f.columns
    if s:
        sq=s.lower()
        match = f['customer'].str.lower().str.contains(sq,na=False)
        if has_equip: match = match | f['equipment'].fillna('').str.lower().str.contains(sq,na=False)
        f=f[match]
    t,c,fu=count_called(f,'customer','recovery'); cbar('Recovery',dbr,t,c,fu)
    # Info banner with branch totals
    branch_decline = f['decline_amount'].sum()
    urgent_ct = len(f[f['action']=='Urgent'])
    st.markdown(f'<div class="info-banner" style="background:#FEF2F2;color:#991B1B;border-left:4px solid #DC2626;"><strong>Recovery Campaign</strong> | {len(f)} declining customers | ${branch_decline:,.0f} at risk' + (f' | {urgent_ct} urgent' if urgent_ct else '') + '<br>Ranked by decline amount. Review equipment and revenue history before calling.</div>',unsafe_allow_html=True)
    ...
    if has_equip and has_acct:
        cheaders_custom = st.columns([0.4,0.4,2.5,1.5,1.3,2.0])
        labels = ['CALLED','F/U','CUSTOMER','EQUIPMENT','REVENUE','NOTES']
        for i,l in enumerate(labels):
            with cheaders_custom[i]: st.markdown(f'<div style="font-size:10px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;padding:4px 0;border-bottom:1px solid #e2e8f0;">{l}</div>',unsafe_allow_html=True)

        for _,r in f.head(100).iterrows():
            cid=str(r['customer']).strip()
            a=r.get('action','') or ''; bc=bm.get(a,'')
            abadge=f'<span class="badge {bc}">{a}</span>' if a else ''
            acct=str(r.get('acct','')).split('.')[0] if pd.notna(r.get('acct')) else ''
            fleet=int(r.get('fleet_size',0)) if pd.notna(r.get('fleet_size')) else 0
            d=r.get('decline_amount',0); ds=f"${abs(d):,.0f}" if pd.notna(d) and d!=0 else ""
            r25=r.get('rev_2025',0); r24=r.get('rev_2024',0); r23=r.get('rev_2023',0)

            lk=f"recovery_{cid}"; log=st.session_state.call_log; ex=log.get(lk,{})
            ic=ex.get('called',False); ifu=ex.get('followup',False); inotes=ex.get('notes','')
            cols=st.columns([0.4,0.4,2.5,1.5,1.3,2.0])
            with cols[0]: nc=st.checkbox("",value=ic,key=f"c_{lk}",label_visibility="collapsed")
            with cols[1]: nf=st.checkbox("",value=ifu,key=f"f_{lk}",label_visibility="collapsed")
            with cols[2]:
                sub_parts = [f"Acct: {acct}" if acct else '']
                if fleet: sub_parts[0] += f" | {fleet} machines"
                st.markdown(f'<div class="customer-name">{cid} {abadge}</div><div class="customer-sub">{sub_parts[0]}</div>',unsafe_allow_html=True)
            with cols[3]:
                eq=str(r.get('equipment','')) if has_equip and pd.notna(r.get('equipment')) else ''
                if eq:
                    eq_items = [e.strip() for e in eq.split(',')][:4]
                    eq_html = '<br>'.join(f'<span style="font-size:12px;color:#334155;">{e}</span>' for e in eq_items)
                    if len(eq.split(',')) > 4:
                        eq_html += f'<br><span style="font-size:11px;color:#94a3b8;">+{len(eq.split(","))-4} more</span>'
                    st.markdown(eq_html, unsafe_allow_html=True)
                else:
                    st.markdown('<span style="color:#cbd5e1;font-size:12px;">No machine data</span>', unsafe_allow_html=True)
            with cols[4]:
                # Revenue history with trend
                rev_lines = [f'<span style="font-size:13px;font-weight:600;color:#DC2626;">-${abs(d):,.0f} decline</span>']
                if pd.notna(r25) and r25 > 0: rev_lines.append(f'<span style="font-size:12px;color:#475569;">2025: ${r25:,.0f}</span>')
                if pd.notna(r24) and r24 > 0: rev_lines.append(f'<span style="font-size:12px;color:#475569;">2024: ${r24:,.0f}</span>')
                if pd.notna(r23) and r23 > 0: rev_lines.append(f'<span style="font-size:12px;color:#64748b;">2023: ${r23:,.0f}</span>')
                st.markdown('<br>'.join(rev_lines), unsafe_allow_html=True)
            with cols[5]:
                nn=st.text_input("notes",value=inotes,key=f"n_{lk}",label_visibility="collapsed",placeholder="Add notes...")
            if nc!=ic or nf!=ifu or nn!=inotes:
                log[lk]={'called':nc,'followup':nf,'notes':nn,'customer_name':cid,'branch_name':dbr,'user':st.session_state.user,'date_updated':datetime.now().strftime('%Y-%m-%d %H:%M')}
                save_log()
    else:
        # Fallback for old CSV without enriched columns
        cheaders(['Called','F/U','Customer','Action','Decline','Notes'])
        for _,r in f.head(100).iterrows():
...
def show_service():
    ...
    has_tier = 'tier' in f.columns
    has_equip = 'equipment' in f.columns
    has_hist = 'history' in f.columns
    if s:
        sq=s.lower()
        match = f['cust_name'].str.lower().str.contains(sq,na=False)
        if has_equip: match = match | f['equipment'].fillna('').str.lower().str.contains(sq,na=False)
        f=f[match]
    t,c,fu=count_called(f,'cust_acct','service')
    cbar(f'{MONTH_NAMES[mo]} Campaign',dbr,t,c,fu)
    total_rev=f['month_revenue'].sum(); strong_ct=len(f[f['tier']=='STRONG']) if has_tier else 0
    st.markdown(f'<div class="info-banner" style="background:#E0F2FE;color:#0C4A6E;border-left:4px solid #0369A1;"><strong>{MONTH_NAMES[mo]} Campaign</strong> | ${total_rev:,.0f} historical {MONTH_NAMES[mo]} revenue | {strong_ct} proven multi-year buyers<br>These customers historically service equipment in {MONTH_NAMES[mo]}. Concentrated buyers have narrow windows - don\'t miss them.</div>',unsafe_allow_html=True)
    st.markdown('<div class="info-banner" style="background:#FFFBEB;color:#92400E;border-left:4px solid #D97706;"><strong>Before Calling:</strong> Verify customer service history and open work orders on NDS.</div>',unsafe_allow_html=True)
    ...
    for _,r in f.head(100).iterrows():
        ...
        tier=str(r.get('tier','')) if has_tier else ''
        ...
        hist=str(r.get('history','')) if has_hist and pd.notna(r.get('history')) else ''
        gl=str(r.get('gl_desc','')) if 'gl_desc' in f.columns and pd.notna(r.get('gl_desc')) else ''
...
def show_consignment():
    ...
    if s:
        sq=s.lower()
        match = f['Customer'].str.lower().str.contains(sq,na=False)
        if 'Equipment' in f.columns: match = match | f['Equipment'].fillna('').str.lower().str.contains(sq,na=False)
        if 'Top_Parts' in f.columns: match = match | f['Top_Parts'].fillna('').str.lower().str.contains(sq,na=False)
        f=f[match]
    ...
    for _,r in f.head(100).iterrows():
        ...
        with cols[2]:
            ...
            tier=str(r.get('Tier','')) if 'Tier' in f.columns and pd.notna(r.get('Tier')) else ''
            tier_colors = {
                'Tier 1A - Elite': ('#1E3A5F','white'),
                'Tier 1B - Prime': ('#1E40AF','white'),
                'Tier 2A - Strong': ('#059669','white'),
                'Tier 2B - Developing': ('#0D9488','white'),
                'Tier 3A - Emerging': ('#D97706','white'),
                'Tier 3B - Watch': ('#9CA3AF','white'),
            }
            if tier and ' - ' in tier:
                tc = tier_colors.get(tier, ('#9CA3AF','white'))
                tier_short = tier.replace('Tier ','T').split(' - ')[0] + ' ' + tier.split(' - ')[1]
                tbadge=f'<span style="background:{tc[0]};color:{tc[1]};padding:2px 8px;border-radius:4px;font-size:10px;font-weight:600;margin-left:6px;">{tier_short.upper()}</span>'
            elif score >= 90: tbadge='<span style="background:#1E3A5F;color:white;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:600;margin-left:6px;">PHASE 1</span>'
            elif score >= 70: tbadge='<span style="background:#059669;color:white;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:600;margin-left:6px;">PHASE 2</span>'
            else: tbadge='<span style="background:#D97706;color:white;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:600;margin-left:6px;">PHASE 3</span>'
            ...
            rep=int(r.get('Repeat_Parts',0)) if 'Repeat_Parts' in f.columns and pd.notna(r.get('Repeat_Parts')) else 0
            sub_parts = [f"Acct: {ac} | Score {score}" + (f" | {rep} repeat parts" if rep > 0 else "")]
            ...
            st.markdown(f'<div class="customer-name">{nm}{tbadge}</div><div class="customer-sub">{sub_line}</div>',unsafe_allow_html=True)
        with cols[3]:
            # Top repeat parts - formatted as compact list
            tp=str(r.get('Top_Parts','')) if 'Top_Parts' in f.columns and pd.notna(r.get('Top_Parts')) else ''
            if tp:
                parts_html = []
                for p in tp.split(' | ')[:4]:
                    parts_html.append(f"<div style='font-size:12px;color:#475569;line-height:1.5;'>{p}</div>")
                rep_total = int(r.get('Repeat_Parts',0)) if 'Repeat_Parts' in f.columns and pd.notna(r.get('Repeat_Parts')) else 0
                if rep_total > 4:
...
REGIONS = {
    'SE (Brian)': ['Cambridge','Gallipolis','Dublin','Heath','Marietta','Brilliant','South Charleston'],
    'NW (Matt)': ['North Canton','Perrysburg','Brunswick','Mentor','Mansfield'],
    'W (Carrie)': ['Monroe','Burlington','Fort Wayne','Indianapolis','Evansville','Holt','Novi'],
}

def build_branch_metrics(log, rec, sn, eda, pts, svc, con, sm):
    """Build comprehensive branch metrics for dashboard."""
    svm = svc[svc['target_month']==sm]
    rows = []
    for brn in sorted(BRANCHES.values()):
        rb = rec[rec['branch']==brn]
        r_dec = rb['decline_amount'].sum()
        r_urg = len(rb[rb['action']=='Urgent'])
        r_ct = len(rb)
        r_called = sum(1 for _,r in rb.iterrows() if log.get(f"recovery_{r['customer']}",{}).get('called'))

        snb = sn[sn['branch']==brn]; eb = eda[eda['branch']==brn]
        cq_ct = len(snb)+len(eb)
        cq_called = sum(1 for _,r in snb.iterrows() if log.get(f"conquest_sn_{r['company']}",{}).get('called'))
        cq_called += sum(1 for _,r in eb.iterrows() if log.get(f"conquest_eda_{r['company']}",{}).get('called'))

        pb = pts[pts['BranchName']==brn]
        p_ct = len(pb)
        p_called = sum(1 for _,r in pb.iterrows() if log.get(f"parts_{r['Customer']}",{}).get('called'))

        sb = svm[svm['branch_name']==brn]
        s_ct = len(sb); s_rev = sb['month_revenue'].sum()
        s_strong = len(sb[sb['tier']=='STRONG']) if 'tier' in sb.columns else 0
        s_called = sum(1 for _,r in sb.iterrows() if log.get(f"service_{r['cust_acct']}",{}).get('called'))

        cb = con[con['Branch']==brn]
        c_ct = len(cb); c_sell = cb['Sell Value'].sum()
        c_called = sum(1 for _,r in cb.iterrows() if log.get(f"consignment_{r['Account']}",{}).get('called'))

        total = r_ct+cq_ct+p_ct+s_ct+c_ct
        called = r_called+cq_called+p_called+s_called+c_called
        pct = int(called/total*100) if total > 0 else 0
        fups = sum(1 for v in log.values() if v.get('followup') and v.get('branch_name')==brn)
        users = len(set(v.get('user','') for v in log.values() if v.get('branch_name')==brn and v.get('user')))

        region = 'Unknown'
        for rname, blist in REGIONS.items():
            if brn in blist: region = rname; break

        rows.append({
            'branch': brn, 'region': region,
            'decline': r_dec, 'urgent': r_urg,
            'r_ct': r_ct, 'r_called': r_called,
            'cq_ct': cq_ct, 'cq_called': cq_called,
            'p_ct': p_ct, 'p_called': p_called,
            's_ct': s_ct, 's_called': s_called, 's_rev': s_rev, 's_strong': s_strong,
            'c_ct': c_ct, 'c_called': c_called, 'c_sell': c_sell,
            'total': total, 'called': called, 'pct': pct,
            'fups': fups, 'users': users,
        })
    return pd.DataFrame(rows)

def show_admin():
    st.markdown('<div class="header-bar"><span class="page-title">Leadership Dashboard</span><span class="page-info">Accountability | Branch Performance | Campaign Intelligence</span></div>',unsafe_allow_html=True)
    ...
    bm = build_branch_metrics(log, rec, sn, eda, pts, svc, con, sm)

    # ── COMPANY SCORECARD ────────────────────────────────────
    total_targets = bm['total'].sum()
    total_called = bm['called'].sum()
    total_fups = bm['fups'].sum()
    co_pct = int(total_called/total_targets*100) if total_targets > 0 else 0
    today = datetime.now().strftime('%Y-%m-%d')
    calls_today = sum(1 for v in log.values() if v.get('called') and v.get('date_updated','').startswith(today))
    active_branches = len(bm[bm['called']>0])
    zero_branches = len(bm[bm['called']==0])

    cols = st.columns(6)
    scorecard = [
        (f"{total_called}", "Calls Made", f"of {total_targets} targets ({co_pct}%)", SE_MAROON, "#FEF2F2"),
        (f"{calls_today}", "Today", "calls logged today", "#059669", "#ECFDF5"),
        (f"{total_fups}", "Follow-Ups", "callbacks pending", "#D97706", "#FFFBEB"),
        (f"{active_branches}", "Active", f"of {len(bm)} branches calling", "#2563EB", "#EFF6FF"),
        (f"{zero_branches}", "Inactive", "branches with zero calls", "#DC2626" if zero_branches > 5 else "#9CA3AF", "#FEF2F2" if zero_branches > 5 else "#F8FAFC"),
        (f"${bm['decline'].sum():,.0f}", "At Risk", f"{len(rec)} declining customers", "#7C3AED", "#F5F3FF"),
    ]
    for i, (num, label, sub, color, bg) in enumerate(scorecard):
        with cols[i]:
            st.markdown(f'<div style="background:{bg};border-left:3px solid {color};border-radius:6px;padding:12px 14px;"><div style="font-size:22px;font-weight:700;color:{color};">{num}</div><div style="font-size:10px;font-weight:600;color:{color};text-transform:uppercase;letter-spacing:0.5px;">{label}</div><div style="font-size:10px;color:#64748b;margin-top:2px;">{sub}</div></div>',unsafe_allow_html=True)
    ...
    rcols = st.columns(3)
    for i, (rname, rbranches) in enumerate(REGIONS.items()):
        rm = bm[bm['region']==rname]
        r_targets = rm['total'].sum()
        r_called = rm['called'].sum()
        r_pct = int(r_called/r_targets*100) if r_targets > 0 else 0
        r_decline = rm['decline'].sum()
        r_active = len(rm[rm['called']>0])
        r_zero = len(rm[rm['called']==0])
        r_fups = rm['fups'].sum()

        color = ['#DC2626','#2563EB','#059669'][i]
        bg = ['#FEF2F2','#EFF6FF','#ECFDF5'][i]

        with rcols[i]:
            zero_flag = f' | <span style="color:#DC2626;font-weight:600;">{r_zero} inactive</span>' if r_zero > 0 else ''
            st.markdown(f"""<div style="background:{bg};border-radius:8px;padding:16px;border-left:4px solid {color};">
                <div style="font-size:14px;font-weight:700;color:{color};margin-bottom:8px;">{rname}</div>
                <div style="font-size:11px;color:#475569;line-height:2;">
                    <strong>Outreach:</strong> {r_called} / {r_targets} ({r_pct}%)<br>
                    <strong>At Risk:</strong> ${r_decline:,.0f}<br>
                    <strong>Branches:</strong> {r_active} active{zero_flag}<br>
                    <strong>Follow-Ups:</strong> {r_fups} pending
                </div>
            </div>""",unsafe_allow_html=True)

    # ── BRANCH ACCOUNTABILITY TABLE ──────────────────────────
    st.markdown("<br>",unsafe_allow_html=True)
    st.markdown("### Branch Accountability")
    st.markdown('<div style="font-size:12px;color:#64748b;margin-bottom:8px;">Sorted by completion %. Branches at 0% need immediate attention. Called/total shown per campaign.</div>',unsafe_allow_html=True)

    display_rows = []
    for _, b in bm.sort_values('pct', ascending=True).iterrows():
        display_rows.append({
            'Region': b['region'].split(' (')[0],
            'Branch': b['branch'],
            'At Risk $': f"${b['decline']:,.0f}",
            'Urgent': b['urgent'],
            'Recovery': f"{b['r_called']}/{b['r_ct']}",
            'Conquest': f"{b['cq_called']}/{b['cq_ct']}",
            'Parts': f"{b['p_called']}/{b['p_ct']}",
            f'Service': f"{b['s_called']}/{b['s_ct']}",
            'Consign': f"{b['c_called']}/{b['c_ct']}",
            'TOTAL': f"{b['called']}/{b['total']}",
            'Progress': f"{b['pct']}%",
            'F/U': b['fups'],
        })
    ...
    zero_list = bm[bm['called']==0].sort_values('decline', ascending=False)
    if len(zero_list) > 0:
        names = ', '.join(f"{r['branch']} (${r['decline']:,.0f})" for _,r in zero_list.head(5).iterrows())
        alerts.append(('INACTIVE BRANCHES', f"{len(zero_list)} branches have logged zero calls: {names}", '#DC2626', '#FEF2F2'))

    # Urgent recovery not contacted
    urg_uncalled = []
    for _, r in rec[rec['action']=='Urgent'].iterrows():
        if not log.get(f"recovery_{r['customer']}",{}).get('called'):
            urg_uncalled.append(r)
    if urg_uncalled:
        top = sorted(urg_uncalled, key=lambda x: -x['decline_amount'])[:5]
        names = ', '.join(f"{r['customer']} (${r['decline_amount']:,.0f})" for r in top)
        alerts.append(('URGENT RECOVERY', f"{len(urg_uncalled)} of {len(rec[rec['action']=='Urgent'])} urgent customers not yet called. Top: {names}", '#991B1B', '#FEF2F2'))

    # High decline + low activity
    for _, b in bm.iterrows():
        if b['decline'] > 400000 and b['pct'] < 5:
            alerts.append(('HIGH RISK', f"{b['branch']}: ${b['decline']:,.0f} at risk, only {b['pct']}% outreach completed", '#DC2626', '#FEF2F2'))

    # Stale follow-ups
    if total_fups > 15:
        alerts.append(('FOLLOW-UPS PILING UP', f"{total_fups} follow-ups pending. These are warm leads losing temperature.", '#D97706', '#FFFBEB'))

    if not alerts:
        st.markdown('<div class="info-banner" style="background:#ECFDF5;color:#065F46;border-left:4px solid #059669;">All branches are actively outreaching. No critical alerts.</div>',unsafe_allow_html=True)
    for severity, msg, color, bg in alerts:
        st.markdown(f'<div class="info-banner" style="background:{bg};color:{color};border-left:4px solid {color};margin-bottom:6px;"><strong>{severity}:</strong> {msg}</div>',unsafe_allow_html=True)

    # ── CAMPAIGN DEEP DIVES ──────────────────────────────────
    st.markdown("<br>",unsafe_allow_html=True)
    st.markdown("### Campaign Deep Dive")
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Recovery", "Conquest", "Parts", "Service", "Consignment"])

    with tab1:
        st.markdown("**Largest Uncontacted Declining Customers**")
        has_eq = 'equipment' in rec.columns
        rows = []
        for _, r in rec.sort_values('decline_amount', ascending=False).iterrows():
            called = log.get(f"recovery_{r['customer']}",{}).get('called',False)
            if not called:
                row = {'Customer':r['customer'],'Branch':r['branch'],'Action':r['action'],'Decline':f"${r['decline_amount']:,.0f}"}
                if has_eq:
                    eq = str(r.get('equipment',''))[:60] if pd.notna(r.get('equipment')) else ''
                    row['Equipment'] = eq
                    row['Fleet'] = int(r.get('fleet_size',0)) if pd.notna(r.get('fleet_size')) else 0
                row['2025 Rev'] = f"${r['rev_2025']:,.0f}" if pd.notna(r.get('rev_2025')) else ''
                rows.append(row)
        if rows:
            st.dataframe(pd.DataFrame(rows).head(20), use_container_width=True, hide_index=True)
        else:
            st.success("All recovery customers have been contacted.")

        st.markdown("<br>**Recovery by Action Type**")
        agg = rec.groupby('action').agg(Count=('customer','count'),Total_Decline=('decline_amount','sum')).reset_index()
        called_by_action = {}
        for act in agg['action']:
            sub = rec[rec['action']==act]
            cc = sum(1 for _,r in sub.iterrows() if log.get(f"recovery_{r['customer']}",{}).get('called'))
            called_by_action[act] = cc
        agg['Called'] = agg['action'].map(called_by_action)
        agg['Remaining'] = agg['Count'] - agg['Called']
        agg['Total_Decline'] = agg['Total_Decline'].apply(lambda x: f"${x:,.0f}")
        agg.columns = ['Action','Customers','Decline $','Called','Remaining']
        st.dataframe(agg, use_container_width=True, hide_index=True)

    with tab2:
        st.markdown("**Highest-Value Warm Targets (SN Match)**")
        rows = []
        for _, r in sn.sort_values('historical_revenue', ascending=False).iterrows():
            called = log.get(f"conquest_sn_{r['company']}",{}).get('called',False)
            if not called:
                rows.append({'Company':r['company'],'Branch':r['branch'],'Hist Revenue':f"${r['historical_revenue']:,.0f}",'Fleet':r.get('total_fleet',''),'Heat':r.get('heat_score','')})
        if rows:
            st.dataframe(pd.DataFrame(rows).head(15), use_container_width=True, hide_index=True)

        st.markdown(f"<br>**Conquest Targets by Branch**")
        cq_rows = []
        for _, b in bm.sort_values('branch').iterrows():
            if b['cq_ct'] > 0:
                cq_rows.append({'Branch':b['branch'],'Region':b['region'].split(' (')[0],'Targets':b['cq_ct'],'Called':b['cq_called'],'Remaining':b['cq_ct']-b['cq_called']})
        st.dataframe(pd.DataFrame(cq_rows), use_container_width=True, hide_index=True)

    with tab3:
        st.markdown("**Parts Campaign by Branch**")
        p_rows = []
        for _, b in bm.sort_values('p_ct', ascending=False).iterrows():
            if b['p_ct'] > 0:
                p_rows.append({'Branch':b['branch'],'Region':b['region'].split(' (')[0],'Targets':b['p_ct'],'Called':b['p_called'],'Remaining':b['p_ct']-b['p_called'],'Pct':f"{int(b['p_called']/b['p_ct']*100) if b['p_ct']>0 else 0}%"})
        st.dataframe(pd.DataFrame(p_rows), use_container_width=True, hide_index=True)

    with tab4:
        st.markdown(f"**{MONTH_NAMES[sm]} Service Targets by Branch**")
        s_rows = []
        for _, b in bm.sort_values('s_rev', ascending=False).iterrows():
            if b['s_ct'] > 0:
                s_rows.append({'Branch':b['branch'],'Region':b['region'].split(' (')[0],'Customers':b['s_ct'],'STRONG':b['s_strong'],'Month Rev':f"${b['s_rev']:,.0f}",'Called':b['s_called'],'Remaining':b['s_ct']-b['s_called']})
        st.dataframe(pd.DataFrame(s_rows), use_container_width=True, hide_index=True)

    with tab5:
        st.markdown("**Consignment Readiness by Tier**")
        if 'Tier' in con.columns:
            c_tiers = con.groupby('Tier').agg(Customers=('Customer','count'),Sell=('Sell Value','sum'),Cost=('Stock Cost','sum')).reset_index()
            if 'Repeat_Parts' in con.columns:
                rp = con.groupby('Tier')['Repeat_Parts'].mean().reset_index()
                c_tiers = c_tiers.merge(rp, on='Tier', how='left')
                c_tiers['Repeat_Parts'] = c_tiers['Repeat_Parts'].round(0).astype(int)
            c_called_by_tier = {}
            for tier in c_tiers['Tier']:
                sub = con[con['Tier']==tier]
                cc = sum(1 for _,r in sub.iterrows() if log.get(f"consignment_{r['Account']}",{}).get('called'))
                c_called_by_tier[tier] = cc
            c_tiers['Called'] = c_tiers['Tier'].map(c_called_by_tier)
            c_tiers['Sell'] = c_tiers['Sell'].apply(lambda x: f"${x:,.0f}")
            c_tiers['Cost'] = c_tiers['Cost'].apply(lambda x: f"${x:,.0f}")
            st.dataframe(c_tiers, use_container_width=True, hide_index=True)
        else:
            st.markdown(f"**{len(con)} consignment candidates** | ${con['Sell Value'].sum():,.0f} total sell value | ${con['Stock Cost'].sum():,.0f} investment")
            c_called = sum(1 for _,r in con.iterrows() if log.get(f"consignment_{r['Account']}",{}).get('called'))
            st.markdown(f"Called: {c_called} / {len(con)}")

    # ── FOLLOW-UP PIPELINE ───────────────────────────────────
    st.markdown("<br>",unsafe_allow_html=True)
    st.markdown("### Follow-Up Pipeline")
    st.markdown('<div style="font-size:12px;color:#64748b;margin-bottom:8px;">Warm leads flagged for callback. Sorted by date.</div>',unsafe_allow_html=True)
    fups = []
    for k, v in log.items():
        if v.get('followup'):
            cpx = k.split('_')[0]; cpx = 'Conquest' if 'conquest' in k else cpx.title()
            fups.append({'Customer': v.get('customer_name',k), 'Branch': v.get('branch_name',''), 'Campaign': cpx, 'User': v.get('user',''), 'Notes': (v.get('notes','')[:80]+'...' if len(v.get('notes',''))>80 else v.get('notes','')), 'Date': v.get('date_updated','')})
    if fups:
        st.dataframe(pd.DataFrame(fups).sort_values('Date', ascending=False), use_container_width=True, hide_index=True)
    else:
        st.info("No follow-ups flagged yet.")
    ...
    with col_a:
        ...
        if uc:
            udata = []
            for name, calls in uc.most_common(20):
                fu = sum(1 for v in log.values() if v.get('user')==name and v.get('followup'))
                notes = sum(1 for v in log.values() if v.get('user')==name and v.get('notes','').strip())
                branches = ', '.join(sorted(set(v.get('branch_name','') for v in log.values() if v.get('user')==name)))
                udata.append({'Name': name, 'Calls': calls, 'F/U': fu, 'Notes': notes, 'Branches': branches})
            st.dataframe(pd.DataFrame(udata), use_container_width=True, hide_index=True)
        else:
    ...
    with col_b:
        ...
        for k, v in log.items():
            if v.get('called'):
                cpx = k.split('_')[0]; cpx = 'Conquest' if 'conquest' in k else cpx.title()
                recent.append({'Customer': v.get('customer_name',k), 'Branch': v.get('branch_name',''), 'User': v.get('user',''), 'Date': v.get('date_updated','')})
        if recent:
            st.dataframe(pd.DataFrame(recent).sort_values('Date', ascending=False).head(25), use_container_width=True, hide_index=True)
        else:
            st.info("No calls logged yet.")
