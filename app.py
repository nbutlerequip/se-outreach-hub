...
def show_consignment():
    ...
    for _,r in f.head(100).iterrows():
        ...
        with cols[2]:
            ...
            tier=str(r.get('Tier','')) if pd.notna(r.get('Tier')) else ''
            tier_colors = {
            ...
            tier_short = tier.replace('Tier ','T').split(' - ')[0] + ' ' + tier.split(' - ')[1] if ' - ' in tier else tier
            ...
            sub_parts = [f"Acct: {ac} | Score {score} | {rep} repeat parts"]
...
def show_admin():
    ...
    with tab1:
        ...
        for _, r in rec.sort_values('decline_amount', ascending=False).iterrows():
            ...
            if not called:
                rows.append({'Customer':r['customer'],'Branch':r['branch'],'Action':r['action'],'Decline':f"${r['decline_amount']:,.0f}",'2025 Rev':f"${r['rev_2025']:,.0f}" if pd.notna(r.get('rev_2025')) else ''})
        if rows:
    ...
    with tab5:
        ...
        c_tiers = con.groupby('Tier').agg(Customers=('Customer','count'),Sell=('Sell Value','sum'),Cost=('Stock Cost','sum'),Avg_Repeat=('Repeat_Parts','mean')).reset_index()
        c_called_by_tier = {}
        ...
        c_tiers['Avg_Repeat'] = c_tiers['Avg_Repeat'].round(0).astype(int)
        c_tiers.columns = ['Tier','Customers','Sell Value','Inv Cost','Avg Repeat Parts','Called']
